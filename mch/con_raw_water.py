...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov 27 10:45:47 2024
...
import re
import warnings
os.chdir("/home/ghielmin/Desktop/data_thesis/mch/")
...
def getStationsInfo(legend_files):
    ...
    for file in legend_files:
        print(file)
        with open(file,'r', encoding = "ISO-8859-1") as legend:
            print("open")
    ...
    stations={}
    for name, st in stations_temp.items():
        coord=st[1].split("/")
...
def convertTime(times,name,j):
    if(int(math.log10(times[0]))+1==8):
        ...
        for i in range(0,len(times)):
            times[i]=datetime.datetime.strptime(str(int(times[i])), '%Y%m%d%H')+datetime.timedelta(hours=tz)
    else:
...
def writeFile(times_table,writer_data,station_header,variables):
...
def printStation(name,station_header,station_data):
    ...
    if(hourly_tables):
        ...
        writeFile(times_table,writer_data,station_header,variables)
    if(daily_tables):
...
output = ''
loaded_data={}
variables_dictionnary={"time":"timestamp","tre200b0":"TA_C","ure200b0":"RH_C","rre150h0":"PSUM","tso050s0":"TSOIL_50_I","rre150b0":"PSUM_C", \
                       "dkl010h0":"DW","tso005s0":"TSOIL_5_I","tso010s0":"TSOIL_10_I","tso020s0":"TSOIL_20_I","gre000h0":"ISWR", \
                       "gre000b0":"ISWR_C","tre200h0":"TA","ure200h0":"RH","prestah0":"P","prestab0":"P_C","tso005hs":"TSOIL_5_I_P10", \
                       "tso010hs":"TSOIL_10_I_P10","tso020hs":"TSOIL_20_I_P10","tso005b0":"TSOIL_5_C","fu3010h0":"VW","fu3010h1":"MAXW", \
                       "dkl010b0":"DW_C","fu3010b0":"VW_C","fu3010b1":"MAXW_C","tso100s0":"TSOIL_100_I","tso100hs":"TSOIL_100_I_P10", \
                       "oli000b0":"ILWR_C","oli000h0":"ILWR","osr000b0":"OSWR_C","osr000h0":"OSWR","hto000sw":"HS","hto000hs":"HS_I","fkl010h0":"VW", \
                       "htoauths":"HS_A","olo000h0":"OLWR","olo000b0":"OLWR_C","ods000h0":"DRAD","ods000b0":"DRAD_C", "hns000hs":"HS_N", \
...
def main():
    ...
    for d in dirs:
      if os.path.isdir(input+"/"+d):
        for file in os.listdir(input+"/"+d):
          if (os.path.isfile(input+"/"+d+"/"+file) and file.endswith("_legend.txt")  and file.startswith("order")) :
            legend_files.append(input+"/"+d+"/"+file)
          if (os.path.isfile(input+"/"+d+"/"+file) and file.endswith("_data.txt")  and file.startswith("order")) :
            data_files.append(input+"/"+d+"/"+file)
